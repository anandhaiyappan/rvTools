...
\: copySessionUrl (void; Event ev)
{
    string url = "rvlink:// -network -networkConnect %s %d -flags syncPullFirst" %
            (myNetworkHost(), myNetworkPort());
    
    string simpleTitle = "RV Sync Session with %s" % myNetworkHost();
   ...
   if (state.pixelInfo neq nil && !state.pixelInfo.empty())
   {
       ...
       for_each (ri; renderedImages())
       {
           if (ri.name == info.name)
           {
               ...
               //  we delay it and do it in the next pre-render event. - alan
               //
...
\: toggleWipe (void;)
{
    ...
    //  everything else) precedence. -- Alan
    //
...
\: syncState (int;)
{
    ...
    if (remoteNetworkStatus() == NetworkStatusOn && !remoteConnections().empty())
    {
...
\: buildMainMenu (Menu; [MenuItem] userMenus = nil)
{
   ...
   Menu exportMenu = Menu {
    ...
    Menu mainMenu_part1 = {
        ...
        {"Control", Menu {
            {"Play",            ~togglePlay,     nil, playState},
            {"Stop",            ~stop,           nil, rangeState},
        ...
        {"Tools", Menu {
            ...
            {"_", nil},
            {"Sync With Connected RVs", ~toggleSync, nil, syncState}
            }},
        {"Audio", Menu {
...
\: defineDefaultBindings (void;)
{
    ...
    bind("key-down-- ", togglePlay, "Toggle Play");
    bind("key-down--1", cycleStackBackward, "Cycle Image Stack Backwards");
    ...
    bind("key-down--0", pixelRelativeScale(1.0), "Scale 1:1");
    ...
    bind("key-down--down", togglePlay, "Toggle Play");
